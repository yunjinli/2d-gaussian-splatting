    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if viewpoint_cam.gt_alpha_mask is not None:
            ...
            loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (
                1.0 - ssim(image * mask, gt_image * mask)
            )
        else:
        ...
        # Weight by alpha so background pixels (where both normals are ~0 and
        # error is ~1) don't create gradients that grow floaters outside the mask.
        alpha_weight = render_pkg["rend_alpha"].detach()
        normal_loss = lambda_normal * (normal_error * alpha_weight).mean()
